  ...
  run_test (fun web_port connection_pipe client ->
    ...
    let%bind () =
      [%expect
        {|
          (escaped
           "SecurityError: Failed to construct 'WebSocket': The port 20 is not allowed.") |}]
    in
    (* immediate failure *)
    let%bind () = dispatch_and_print (sprintf "wss://localhost:%d/" web_port) in
    let%bind () = [%expect {| (escaped "Connection close with error") |}] in
    (* immediate failure *)
    let%bind () =
          ...
          ((rpc_error (Connection_closed ("RPC transport stopped")))
           (connection_description
            ("Client connected via WS"
             ((scheme (ws)) (host (localhost)) (port <opaque>) (path /))))
